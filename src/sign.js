...
function genSign(service, method, pathname, options = {}) {
  ...
  const sign = util.base64HmacSha1(data.join('&'), service.password);
  return `UPYUN ${service.operator}:${sign}`;
}
...
function getPolicyAndAuthorization(service, params = {}) {
  ...
  const policy = util.base64(JSON.stringify(params));
  const authorization = genSign(service, 'POST', `/${service.name}`, {
    policy,
